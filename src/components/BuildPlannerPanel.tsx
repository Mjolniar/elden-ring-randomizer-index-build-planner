        ...
        <div className="build-detail-panel">
          ...
          <div className="planner-note">
            {isUserBuild
              ...
              : 'The catalog uses paraphrased build metadata from the saved reference page. Matched items are sorted by rough area progression within the loaded spoiler log, not by a solved route.'}
          </div>
                    ...
                    <tr key={`${selectedBuild.id}-${index}-${match.requirement.kind}-${match.requirement.name}`} className={`record-row${record?.isKeyItem ? ' key-item' : ''}${isAcquired ? ' acquired' : ''}`}>
                      ...
                      {!spoilerSettings.spoilerMode ? (
                        ...
                        spoilerSettings.showHint && (
                          <td>{record ? generateHint(record, spoilerSettings.hintDifficulty) : (match.isFreeform ? freeformLocationText(match.requirement.kind) : 'Not found in loaded spoiler log')}</td>
                        )
                      )}
